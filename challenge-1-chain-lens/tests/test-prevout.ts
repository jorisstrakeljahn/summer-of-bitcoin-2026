 ...
 *
 * Why: Prevouts must be matched by (txid, vout) — not by array position.
 * The grader explicitly tests unordered prevouts and error cases.
 */

import { readFileSync } from "fs";
import { parseTransaction } from "../src/lib/tx-parser.js";
import { matchPrevouts } from "../src/lib/prevout.js";
import { describe, test, assertEqual, assertThrows } from "./helpers.js";
...
// ---------------------------------------------------------------------------
// All fixtures must match successfully
// ---------------------------------------------------------------------------
...
describe("Prevout matching: all fixtures", () => {
  for (const name of names) {
    test(`${name} — all prevouts matched`, () => {
...
// ---------------------------------------------------------------------------
// Fee computation (input_total - output_total = fee)
// ---------------------------------------------------------------------------
...
describe("Prevout matching: fee computation", () => {
  for (const [name, expected] of Object.entries(fees)) {
    test(`${name} fee = ${expected} sats`, () => {
      const f = loadFixture(name);
      const p = parseTransaction(f.raw_tx);
      const m = matchPrevouts(p.inputs, f.prevouts);
...
describe("Prevout matching: error cases", () => {
