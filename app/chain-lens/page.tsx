...
import type { TransactionReport, BlockReport, BlockSummary } from "@/lib/chain-lens/types";
import { Header } from "@/components/chain-lens/Header";
import { InputPanel } from "@/components/chain-lens/InputPanel";
import { TransactionResult } from "@/components/chain-lens/TransactionResult";
import { BlockSummaryList } from "@/components/chain-lens/BlockSummaryList";
import { BlockResult } from "@/components/chain-lens/BlockResult";
import { ErrorDisplay } from "@/components/chain-lens/ErrorDisplay";
...
export default function ChainLensPage() {
  ...
  async function handleAnalyzeFixture(name: string) {
    ...
    try {
      const res = await fetch(`/api/chain-lens/analyze-fixture?name=${encodeURIComponent(name)}`);
  ...
  async function handleAnalyzeTx(fixtureJson: string) {
    ...
    try {
      ...
      const res = await fetch("/api/chain-lens/analyze", {
        method: "POST",
  ...
  async function handleAnalyzeBlock(blk: File, rev: File, xor: File) {
    ...
    try {
      ...
      const res = await fetch("/api/chain-lens/analyze-block", {
        method: "POST",
        body: formData,
      });
      const result = await res.json();
  ...
  async function handleLoadBlock(session: string, blocks: BlockSummary[], hash: string) {
    ...
    try {
      const res = await fetch(`/api/chain-lens/block-report?session=${encodeURIComponent(session)}&hash=${encodeURIComponent(hash)}`);
      const result = await res.json();
      if (result.ok !== false) {
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 space-y-8 px-4 py-8 sm:px-6">
        ...
        {state.mode === "block-loading" && (
          <>
            <BlockSummaryList blocks={state.blocks} onSelectBlock={() => {}} disabled />
